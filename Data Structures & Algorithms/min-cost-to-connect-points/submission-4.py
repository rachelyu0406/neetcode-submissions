# optial solution Prim's algo
# optimal because number of distances you need to calculate decreases
# and also no heappush/pop removes logn factor
class Solution:
    def minCostConnectPoints(self, points: List[List[int]]) -> int:
        ...
        for _ in range(n):
            curr = -1
            for i in range(n):
                if i not in visited and (curr == -1 or minDist[i] < minDist[curr]):
                    curr = i
            visited.add(curr)
            total += minDist[curr]
            xi, yi = points[curr]
